...
public interface SlnaFunctionaryRepository extends JpaRepository<SlnaFunctionary, Integer>{
	...
	@Query(value = "SELECT f.ppr_slna_fun_id, f.ppr_inst_str_id, f.slna_fun_name, d.designation_name, q.qualification_name, f.work_allocation, "
			...
			+ "e.office_name, e.address, e.work_exp_yrs, e.work_exp_days, e.work_details FROM ppr_slna_institutional_structure i inner join ppr_slna_functionary f "
